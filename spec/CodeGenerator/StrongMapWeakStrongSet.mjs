import StrongMapWeakStrongSet from "../generated/StrongMapWeakStrongSet.mjs";
import ToHoldRefsMatchers from "../support/toHoldReferences.mjs";

describe("CodeGenerator(StrongMapWeakStrongSet.mjs)", () => {
  ...
  beforeEach(() => {
    refSet.clear();
    testSet = new StrongMapWeakStrongSet();
  });

  it("class is frozen", () => {
    expect(Object.isFrozen(StrongMapWeakStrongSet)).toBe(true);
    expect(Object.isFrozen(StrongMapWeakStrongSet.prototype)).toBe(true);
  });
  ...
  it("adding two keys with a constant first key", () => {
    ...
    expect(() => testSet.clear()).not.toThrow();
    expect(testSet.has(key3, key1, key4)).toBe(false);
    expect(testSet.has(key3, key2, key4)).toBe(false);
  });
  ...
  it("invoking .addSets() and its full-set friends", () => {
    ...
    expect(testSet.hasSet(key1, key4)).toBe(false);
    expect(testSet.hasSet(key2, key4)).toBe(false);
    expect(testSet.hasSet(key3, key4)).toBe(true);
