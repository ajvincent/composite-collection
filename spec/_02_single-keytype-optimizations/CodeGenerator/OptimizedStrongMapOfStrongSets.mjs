...
describeForAllThree("OptimizedStrongMapOfStrongSets", (modules, mapMod, setMod) => {
  const {
  ...
  beforeEach(() => {
    mapOfSets = new OptimizedStrongMapOfStrongSets;
    firstMapKey = mapMod([new MockImportable]);
    firstSetKey = [new MockImportable, new MockImportable];
    secondMapKey = mapMod([new MockImportable]);
    secondSetKey = [new MockImportable, new MockImportable];
  });
  afterEach(() => {
    mapOfSets = null;
    firstMapKey = null;
  ...
  it("class is frozen", () => {
    expect(Object.isFrozen(OptimizedStrongMapOfStrongSets)).toBe(true);
    expect(Object.isFrozen(OptimizedStrongMapOfStrongSets.prototype)).toBe(true);
  });

  it("class only exposes public methods", () => {
    expect(Reflect.ownKeys(OptimizedStrongMapOfStrongSets.prototype)).toEqual([
      "constructor",
      "size",
      "getSizeOfSet",
      "mapSize",
  ...
  describe("to hold references strongly", () => {
    ...
    it("as the set key", async () => {
      await expectAsync(
        key => {
          const setKey = [new MockImportable(key), new MockImportable];
