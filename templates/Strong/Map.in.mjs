...
${docs.buildBlock("forEachMap", 2)}
  forEach(
    callback: __${defines.className}_forEachCallbackMap__<${defines.tsMapTypes.join(", ") + ", " + defines.tsValueType}>,
    thisArg: unknown
  ) : void
  {
    this.#root.forEach((valueAndKeySet) => {
      const args: [${defines.tsValueType}, ${defines.tsMapTypes.join(", ")}, this] = [
        valueAndKeySet.value,
        ...valueAndKeySet.keySet,
        this
      ];
...
${docs.buildBlock("set", 2)}
  set(${defines.tsMapKeys}, ${defines.tsValueKey}) : this {${invokeValidate}
${defines.validateValue ? `
